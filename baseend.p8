...
function start()
 ...
 if textindex>100 then mode="win" textindex=0 resetmode() end
end
...
-- Hello 
-- Im trying to  create the ending animation where she will break the wall
-- and reapear in the cafereria like jumping from the middle part and landing 
-- and walk to the chair again and have some text to speech but having problem with
-- the map draw and with the player animation is kinda bad 
-- little bit of feedback plz 
-- Alexis



	--Not working
spr(8 , pendx , pendy - 16)

-- timing the explosion
local timeson = time()

-- explosion code
if bombflag == false then
		for i = 1,30 do 
		add(parts , {
		x=0,
		y=63,
